...
CREATE TRIGGER `TGR_UPDATE_BALANCE` AFTER INSERT ON `JWL_BALANCE_HIS` 
    FOR EACH ROW 
BEGIN
   DECLARE existOpenBalance FLOAT(10,2) DEFAULT 0.00;
   SELECT BALANCE into existOpenBalance FROM JWL_BALANCE;
  	IF NEW.BALANCE_TYPE =1 THEN
		UPDATE JWL_BALANCE SET BALANCE = NEW.BALANCE + existOpenBalance, UPDATED_ON = CURDATE();
	END IF;  
END;
$$
