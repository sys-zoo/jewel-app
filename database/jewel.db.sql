DROP DATABASE JWELLERY_SHOP_DB_V1;
CREATE DATABASE JWELLERY_SHOP_DB_V1;
USE JWELLERY_SHOP_DB_V1;
...
CREATE TABLE JWL_STOCK(
	...
	SALES_WEIGHT FLOAT(10,2) DEFAULT '0.00',
	SHOP_ITEM_WEIGHT FLOAT(10,2) DEFAULT '0.00',
	IS_DEL TINYINT(1) DEFAULT 0,
	CREATED_ON TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
	CREATED_BY VARCHAR(30),
	UPDATED_ON TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE JWL_STOCK_HIS(
	...
	OTHERS_AMT FLOAT(10,2) DEFAULT '0.00',
	IS_OLD_MATERIAL TINYINT(1) DEFAULT 1,
...
CREATE INDEX IDX_STOCK_CREATED_ON ON JWL_ORDER(CREATED_ON);
...
INSERT INTO JWL_ACTION_TYPE VALUES(2, 'ADVANCE AMOUNT');
INSERT INTO JWL_ACTION_TYPE VALUES(3, 'MATERIAL SENT TO GOLD SMITH');
INSERT INTO JWL_ACTION_TYPE VALUES(4, 'MATERIAL RECEIVED FROM GOLD SMITH');
INSERT INTO JWL_ACTION_TYPE VALUES(5, 'WAGES');
INSERT INTO JWL_ACTION_TYPE VALUES(6, 'GOLD SMITH MAKING CHARGE');
INSERT INTO JWL_ACTION_TYPE VALUES(7, 'ORDER HOLD');
INSERT INTO JWL_ACTION_TYPE VALUES(8, 'ORDER CANCELLED');
...
CREATE INDEX IDX_ORDER_TRACK_CREATED_ON ON JWL_ORDOR_TRACK(CREATED_ON); 
...
CREATE TRIGGER `TGR_UPDATE_STOCK` AFTER INSERT ON `JWL_STOCK_HIS` 
    FOR EACH ROW 
BEGIN
   DECLARE existWeight FLOAT(10,2) DEFAULT 0.00;
   SELECT INSTOCK_WEIGHT into existWeight FROM JWL_STOCK;
   UPDATE JWL_STOCK SET INSTOCK_WEIGHT = NEW.WEIGHT + existWeight;
END;
$$
...
BEGIN
   ...
   DECLARE existShopWeight FLOAT(10,2) DEFAULT 0.00;
   DECLARE existSalesWeight FLOAT(10,2) DEFAULT 0.00;
   ...
   SELECT INSTOCK_WEIGHT,OUTSTOCK_WEIGHT,SHOP_ITEM_WEIGHT,SALES_WEIGHT into existInStockWeight,existOutStockWeight,existShopWeight,existSalesWeight FROM JWL_STOCK;
   SELECT ORDER_TYPE,ADVANCE_AMT,WAGES_AMT,MAKING_AMT INTO orderType,existAdvanceAmount,existWagesAmount,existMakingAmount FROM JWL_ORDER WHERE ORDER_ID=NEW.ORDER_ID;
   IF NEW.ACTION_TYPE_ID =2 THEN
     UPDATE JWL_ORDER SET ADVANCE_AMT = existAdvanceAmount + NEW.AMOUNT WHERE ORDER_ID=NEW.ORDER_ID;
   ELSEIF NEW.ACTION_TYPE_ID =3 THEN
     UPDATE JWL_STOCK SET INSTOCK_WEIGHT = existInStockWeight - NEW.WEIGHT, OUTSTOCK_WEIGHT = existOutStockWeight + NEW.WEIGHT;
	 UPDATE JWL_ORDER SET STATUS = 2 WHERE ORDER_ID=NEW.ORDER_ID;
   ELSEIF NEW.ACTION_TYPE_ID =4 THEN
    IF orderType =0 THEN
		UPDATE JWL_STOCK SET SALES_WEIGHT = NEW.WEIGHT + existSalesWeight;
	ELSE
		UPDATE JWL_STOCK SET SHOP_ITEM_WEIGHT = NEW.WEIGHT + existShopWeight;
	END IF;
	 UPDATE JWL_STOCK SET OUTSTOCK_WEIGHT = existOutStockWeight - NEW.WEIGHT;
   ...
   ELSEIF NEW.ACTION_TYPE_ID =6 THEN
  	 UPDATE JWL_ORDER SET MAKING_AMT = existMakingAmount + NEW.AMOUNT WHERE ORDER_ID=NEW.ORDER_ID;
   ELSEIF NEW.ACTION_TYPE_ID =7 THEN
	 UPDATE JWL_ORDER SET STATUS = 5 WHERE ORDER_ID=NEW.ORDER_ID;
